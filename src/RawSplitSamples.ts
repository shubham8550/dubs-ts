...
export class RawSplitSamples {
  constructor(public audio:Audio, public segment_time = 9,public  tortoiseVoicesDir = path.join("C:\\Users\\shubh\\anaconda3\\tortoise-tts\\tortoise\\voices\\dubber")) {
    this.write()
  }
  ...
  async write() {
    ...
    const command = `ffmpeg -i ${(await this.audio.wavAudio()).filepath } -f segment -c:a pcm_f64le -ar 22050 -segment_time ${this.segment_time} -c copy ${path.join(this.tortoiseVoicesDir, 'out%03d.wav')}`;
    execSync(command);
  }
}
